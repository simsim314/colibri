#!/usr/bin/env python3
"""Interactive sorted-weight viewer for stacked GGUF MoE expert tensors.
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Load and plot sorted dequantized weights from stacked GGUF MoE experts."
    )
    p.add_argument("model", type=Path)
    p.add_argument(
        "--library",
        type=Path,
        default=Path(__file__).resolve().with_name("libgguf_weight_bridge.so"),
    )
...
def main() -> int:
    ...
    if not args.library.is_file():
        print(
            ...
            "Run ./build_gguf_weight_viewer.sh inside the Colibri c directory.",
            file=sys.stderr,
        )
        return 2
